...
//                public static List<BookInfo> books = new ArrayList<>();
	...
	public static void bookToCsv() throws Exception {
		...
		BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("book.csv",true), "EUC-KR"));
		...
		while (iterator.hasNext()) {
			...
			writer.append(String.valueOf(book.getId())).append(",").append(book.getTitle()).append(",")
					.append(book.getWriter()).append(",").append(book.getPublisher()).append(",")
					.append(String.valueOf(book.getisAvailable())).append("\n");
		}
	...
	public static void csvToUser() throws Exception {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
			...
			while ((line = br.readLine()) != null) {
				...
				UserInfo user = new UserInfo(signupdate, name, age, address, phonenumber, birthday, sex);
	...
	public static void UserToCsv() throws Exception {
		...
		try {
			BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("user.csv",true), "EUC-KR"));
			...
			while (iterator.hasNext()) {
				...
				writer.append(user.getSignupdate()).append(",").append(user.getName()).append(",")
						.append(String.valueOf(user.getAge())).append(",").append(user.getAddress()).append(",")
						.append(user.getPhonenumber()).append(",").append(user.getBirthday()).append(",")
	...
	public static void csvToCheckOut() throws Exception {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
			...
			while ((line = br.readLine()) != null) {
				...
				boolean rentExtentionState = Boolean.parseBoolean(data[4]);

//				 SimpleDateFormat 객체 생성
				SimpleDateFormat dateFormat = new SimpleDateFormat("EEE MMM dd HH:mm:ss z yyyy", Locale.ENGLISH);
				Date newCheckoutDate = dateFormat.parse(checkoutdate);
				Date newDeadlineDate = dateFormat.parse(deadlinedate);

				CheckoutInfo checkout = new CheckoutInfo(name, title, newCheckoutDate, newDeadlineDate,
						rentExtentionState);
...
//		FileWriter writer = new FileWriter("checkout.csv");
		BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("checkout.csv",true), "EUC-KR"));
		...
		while (iterator.hasNext()) {
			...
			writer.append(checkout.getName()).append(",").append(checkout.getTitle()).append(",")
					.append(String.valueOf(checkout.getCheckoutdate())).append(",")
					.append(String.valueOf(checkout.getDeadlinedate())).append(",")
					.append(String.valueOf(checkout.isRentExtentionState())).append("\n");
		}
